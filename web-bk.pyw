...
class WebWallpaper(QMainWindow):
    def __init__(self):
        super().__init__()
        self.url = self.get_url_from_json()
        self.web_view = QWebEngineView(self)
        self.original_wallpaper = self.get_current_wallpaper()
        self.initUI()

    def get_url_from_json(self):
        try:
            with open("url.json", "r") as f:
                data = json.load(f)
                return QUrl(data.get("url", ""))
        except FileNotFoundError:
            print("Error: url.json not found. Using default blank page.")
            return QUrl("about:blank")
        except json.JSONDecodeError:
            print("Error: url.json is invalid. Using default blank page.")
            return QUrl("about:blank")
